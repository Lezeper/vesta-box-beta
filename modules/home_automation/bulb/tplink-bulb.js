...
```
   */
  close () {
    if(client.has(this.ip)) {
      client.get(this.ip).close();
      client.delete(this.ip);
      return 0;
    } else { return 1 }
  }
  ...
  send (msg) {
    return new Promise((resolve, reject) => {
      ...
      socket.send(message, 0, message.length, 9999, this.ip, (err, bytes) => {
        if (err) {
          return reject(err)
        }
      })
